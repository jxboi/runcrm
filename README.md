...
- **CRM API**: plain REST under `/api/contacts`, `/api/deals`, `/api/activities`, `/api/tasks`, `/api/agents`, `/api/messages`, `/api/proposals`, `/api/mutations/undo` — usable without the chat.
...
Agent memory that persists corrections · scheduled routines and watchdogs · the rest of the autonomy ladder (advisor diffs, per-action policies) · connectors for email and calendar · per-account threads
